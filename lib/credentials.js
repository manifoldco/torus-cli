...
function unpackCredentials (provided, services) {
  ...
  provided.forEach((component) => {
    ...
    var service = services.find((service) => {
      return (service.service.name === component.meta.service);
    });
    ...
    if (!credTree[serviceName][component.meta.id]) {
      credTree[serviceName][component.meta.id] = {};
    }

    credTree[serviceName][component.meta.id][component.name] = component.value;
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    return Promise.all([
      ...
      return credentialsApi.list(opts).then((provided) => {
        ...
        var required = {};
        var credTree = unpackCredentials(provided, services);
        var serviceMap = {};
        ...
        results.forEach((result) => {
          result.acquired.forEach((credential) => {

            credCount += 1;
            credential.components.forEach((component) => {
              actions.push(credentialsApi.create({
                session_token: box.get('session_token'),
                type: 'env',
